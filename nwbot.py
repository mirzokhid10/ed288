...
# Temporary state for admins building a post: {user_id: {step, text, button_label}}
post_states: dict[int, dict] = {}

# Published post configs: {post_id: {url, button_label, created_at}}
# post_id is a short hex token safe for CallbackButton payloads
...
async def save_post_configs():
    ...
    try:
        ...
        logger.info(f"💾 Saved {len(post_configs)} post configs to {POST_CONFIGS_FILE}")
    except Exception as e:
        logger.error(f"❌ Error saving post configs: {e}", exc_info=True)
...
# ========================================
# POST CREATION — subscription-gated channel posts
# ========================================
...
async def handle_callback(event: MessageCallback):
    ...
    if payload.startswith("unlock_"):
        post_id = payload.replace("unlock_", "")
        logger.info(f"🔓 User {user_id} ({user_name}) clicked unlock button for post {post_id}")
        
        # Check if post exists
        if post_id not in post_configs:
            await event.bot.send_message(
                chat_id=chat_id,
                text="❌ Пост недоступен или был удалён."
            )
            return
        ...
        if await is_subscribed(user_id):
            ...
            logger.info(f"✅ User {user_id} is subscribed - granting access to {config['url']}")
            
            unlock_button = AttachmentButton(
                type=AttachmentType.INLINE_KEYBOARD,
                payload=ButtonsPayload(
                    buttons=[
                        [LinkButton(text="🔗 Перейти к материалу", url=config["url"])]
                    ]
                )
            )
            ...
            await event.bot.send_message(
                ...
                attachments=[unlock_button]
            )
        else:
            # ❌ User NOT subscribed - show gate
            logger.info(f"❌ User {user_id} is NOT subscribed - showing gate")
            
            await event.bot.send_message(
                chat_id=chat_id,
                text=(
                    "❌ Простите, но этот материал доступен только для подписчиков канала.\n\n"
                    "📢 Подпишитесь на канал и попробуйте ещё раз!"
                ),
                attachments=[get_subscribe_keyboard()]
            )
...
# ========================================
# POST WIZARD HANDLER (must be BEFORE general message handler)
# ========================================

@dp.message_created()
async def post_wizard_handler(event: MessageCreated):
    """Handle post creation wizard steps"""
    ...
    # ========================================
        ...
        await event.message.answer(
            ...
            "Пример: https://example.com/guide"
        )
        return
    ...
    # ========================================
        ...
        post_id = secrets.token_hex(6)  # 12 characters
        ...
        try:
            ...
            # Create callback button for the post
...
# ========================================
# VIDEO PROCESSING HANDLER (must be AFTER post wizard)
# ========================================
...
async def handle_video_message(event: MessageCreated):
    ...
    # Skip if user is in post creation flow
    if user_id in post_states:
        return  # Already handled by post_wizard_handler
